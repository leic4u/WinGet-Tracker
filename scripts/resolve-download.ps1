function Resolve-Download($config, $version, $urlVersion = $null, $checkverData = $null) {
    ...
    foreach ($arch in $config.autoupdate.architecture.Keys) {
        $archTemplates = $config.autoupdate.architecture[$arch]
        if ($null -eq $archTemplates) {
            continue
        }

        $templates = @()
        if ($archTemplates -is [System.Collections.IEnumerable] -and -not ($archTemplates -is [string]) -and -not ($archTemplates -is [System.Collections.IDictionary])) {
