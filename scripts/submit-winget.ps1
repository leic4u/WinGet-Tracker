...
foreach ($item in $updates) {
    try {
        ...
        # ========================================
        # 第二步：如果第一步没有找到 PR，则下载软件包并提取内置版本号
        # ========================================
        $processedDownloads = @()
        $tempFiles = @()
        ...
        if (-not $skipDownload) {
            Write-Log "Step 2: Processing $id -> $version"
            ...
            # 打印解析出的下载链接
            foreach ($d in $downloads) {
                Write-Log "  Download URL for $($d.arch): $($d.url)"
            }
            ...
            # ========================================
            # 第三步：使用 installer 内置版本号再次检查 PR 是否已存在
            # ========================================
        ...
        # ========================================
        # 第四步：如果 PR 不存在，则提交到 winget-pkgs
        # ========================================
        $submitSuccess = $false
        $packageNotFound = $false

        if ($shouldSubmit) {
            Write-Log "Step 4: Submitting to winget-pkgs..."
...
if ($hasFatalError) {
    ...
    Write-Log "Submission process completed successfully."
    exit 0
}
